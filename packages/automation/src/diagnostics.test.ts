...
describe("diagnostic sanitization", () => {
  it("redacts likely secrets, tokens, and private prompts from nested values", () => {
    const sanitized = sanitizeDiagnosticValue({
      ...
      log: "request failed with OPENAI_API_KEY=sk-test-123 and bearer ghp_abcdefghijklmnopqrstuvwxyz123456",
      args: ["--api-key", "sk-test-456", "--target", "64x64"],
    });
...
describe("diagnostic reports", () => {
  it("creates a deterministic sanitized failure report with recovery hints", () => {
    const failure = automationError("invalid_options", "Invalid API key sk-test-123 in prompt private text.", 2, {
      apiKey: "sk-test-123",
    ...
    const report = createDiagnosticReport({
      ...
      error: failure.error,
      options: { target: "bad", apiKey: "sk-test-123" },
      paths: { inputPath: "C:/assets/hero.png", outputPath: "C:/out/hero.png" },
      metadata: { argv: ["fix", "hero.png", "--api-key", "sk-test-123"] },
    });
    ...
    expect(report).toMatchObject<DiagnosticReport>({
      ...
      error: {
        ...
        message: "Invalid API key [REDACTED] in prompt [REDACTED].",
        exitCode: 2,
