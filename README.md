...
- Timeline/player controls for sheet-like modes: choose detected row clips, scrub frames, step previous/next, play/pause through frames with `requestAnimationFrame`, set fallback FPS, edit selected-frame duration, toggle looping, normalize frame preview/export canvases, rename detected row clips, and edit per-clip FPS/loop metadata. Frame `durationMs` and detected row clips export into the JSON manifest.
...
- Sheet controls are partly automatic for clear row-based and outlined-grid sheets, but true uneven gutter normalization, per-engine normalized atlas options, onion skin, OCR/label-aware animation naming, and imported timesheet editing are not implemented yet.
...
3. Timeline and player: add onion-skin options, ping-pong playback, richer timesheet editing, and row-label-aware animation naming.
...
The next best implementation step is adding onion-skin or ping-pong playback, followed by deeper uneven-gutter correction for sheets without visible cell outlines.
