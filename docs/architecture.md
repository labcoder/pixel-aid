...
11. For single sprites, the core applies block downsampling, alpha cleanup, optional outline padding for auto-cropped single sprites, optional outline cleanup, palette extraction/remapping, and then the `repairExisting`-only post-palette repairs when eligible: source-coordinate semantic fringe replacement and neutral-gray shell normalization. Those repairs are single-image only and do not run for `none`, `add`, or sheet-frame fixes.
