...
14. The timeline player uses those frame records to scrub, step, and play frames with a `requestAnimationFrame` loop. Detected row animations can be selected, renamed, and given per-clip FPS/loop metadata. A web-side normalization helper can preview frames in a shared pivot-aligned canvas without changing the exported PNG yet.
15. Export passes the current frame metadata and detected row animations to `packages/exporters`, creates a PNG in browser canvas and a generic JSON manifest, then bundles both files into a ZIP.
...
- Sprite player: consume current frame metadata and detected animation tags first, then add export repacking for normalized canvases, onion skin, ping-pong playback, and editable per-frame timing.
