...
| `demo-uneven-labeled-sheet` | `uneven-gutter-labeled-sheet` | Animation sheet | Row labels, uneven gutters, and variable row lengths can confuse sheet detection and timeline setup. | Manual 1x grid, 48x42 output cells, 3 rows, 6 columns, 84px label margin, sheet-locked auto palette, no dithering, 24 colors. | 3-row idle/walk/jump sheet with normalized output cells, preserved source boxes, and visible uneven-gutter warnings. |
