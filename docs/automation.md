...
Phase 8 cross-surface checks use `createRobustEvidenceDryRun` from `@pixelaid/automation`, or the CLI wrapper `pixelaid compare-robust`. Both force the frozen automatic comparison contract, write Classic and Robust Guarded PNGs, and hash decoded RGBA pixels with dimensions included. The resulting JSON contains no source bytes, filename, path, URL, or prompt. These records are explicitly procedural and cannot count as human preference evidence.
