...
flowchart TD
  ...
  O --> Q["remapToPalette"]
  P --> Q
  Q --> R["PixelFixResult diagnostics and metrics"]
```
...
- The single-image path resolves a grid, preprocesses, downsamples, cleans, resolves a palette, remaps, then assembles result diagnostics (`fix.ts:41-224`).
...
| Palette | Resolves palette from the cleaned single image. | Resolves one sheet palette from the packed sheet, passing frames for drift diagnostics. | `fix.ts:168-188`, `fix.ts:337-358`, `palette.ts:124-201` |
| Result | Adds grid, metrics, settings, and diagnostics. | Adds packed sheet image, synthetic sheet grid, metrics, settings, and diagnostics. | `fix.ts:196-224`, `fix.ts:362-401` |
...
- Single-image diagnostics are assembled at `fix.ts:210-220`. This path can include detailed halo, outline, and line-cleanup diagnostics because it calls the detailed helpers directly.
- Sheet-frame diagnostics are assembled at `fix.ts:391-397`. This path merges alpha and morphology across frames and includes the sheet palette, but it does not currently return per-frame halo, outline, or line-cleanup detail.
...
| Cleanup | `cleanup.removeHalos`, `denoiseStrength`, `morphology`, `outline*`, `lineCleanup`, `removeOrphans`, `jaggyCleanup`, `preserveSinglePixelDetails` | Edge, matte, outline, denoise, line, and morphology passes. |
