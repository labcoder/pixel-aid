...
After reconstruction, `packaging.canvasMode` selects tight `content`, the reconstructed `native` composition, or an `exact` canvas. Framing can preserve source composition, pack the subject, or fit it; scaling can retain native pixels, use an integer multiple, or explicitly resample; and anchors control placement. Preserved composition uses normalized source geometry, so a background-removed subject keeps the same relative padding as the original source. Keeping or removing the background therefore does not silently change output geometry.

`outputSizeMode: "detected" | "source" | "exact"` remains as a compatibility policy for existing core callers that omit the new contract. New product and automation surfaces should expose reconstruction and packaging separately. Neither contract changes asset classification, background-removal policy, outline handling, palette generation, fringe correction, or the selected block downscale method.
...
Robust mode also runs independently designed autocorrelation and run-spacing proposers. Their candidates enter a bounded per-axis union with proposer, evidence-family, independence-group, rank, and harmonic provenance. The original integrated detector still supplies the incumbent. Independent candidates can only override it when the same proposer supports both axes, proposal strength is high, each proposed cell has enough source samples, and provisional cell-coherence/distillability improves materially.

At most twelve candidate grids receive a cheap early reconstruction score, capped at 1,024 sampled cells and nine samples per cell. At most two independent challengers then join the incumbent in a full three-hypothesis confirmation pass. These temporary reconstructions use block statistics only; they never enter the output pipeline and do not run palette reduction, alpha cleanup, background removal, halo repair, outlines, or morphology. The legacy finalist path still combines detector prior, within-cell compactness, cross-cell separation, a blur-tolerant residual, and a small complexity penalty with a `0.03` switch threshold. Independent arbitration uses direct cell evidence with a stricter `0.04` margin plus proposer and sampling guards.

Robust diagnostics remain bounded and serializable. They include per-axis period, cell count, boundary offset, boundary coverage/density, run agreement and reliability, harmonic advantage, blur score/weight, pair margin, confidence label, crop policy, full-canvas cell count, proposal provenance, decision basis, and at most three final reconstruction-score summaries. A heavily blurred, drifted, noisy source may still be ambiguous. In that case the detector is expected to keep the incumbent, report low confidence, and expose the insufficient score margin rather than claim a reliable exact size.

This strategy does not classify assets, remove backgrounds, choose alpha cleanup, change palette settings, or alter cleanup passes. It only supplies a candidate reconstruction grid after the caller has established an eligible asset type and mode. The temporary scorer does not blur or resample the final output; the selected grid still flows through PixelAid's normal block-aware downsampling.
