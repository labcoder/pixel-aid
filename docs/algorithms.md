...
Robust mode provisionally reconstructs at most three candidate grids for scoring. These temporary reconstructions use block statistics only; they never enter the output pipeline and do not run palette reduction, alpha cleanup, background removal, halo repair, outlines, or morphology. Scoring combines the detector prior, within-cell compactness, cross-cell separation, a blur-tolerant residual, and a small complexity penalty. A candidate replaces the detector incumbent only when its score leads by at least `0.03`; otherwise the incumbent stays selected and an ambiguous decision lowers confidence.

Robust diagnostics remain bounded and serializable. They include per-axis period, cell count, boundary offset, boundary coverage/density, run agreement and reliability, harmonic advantage, blur score/weight, pair margin, confidence label, crop policy, full-canvas cell count, and at most three reconstruction-score summaries. A heavily blurred, drifted, noisy source may still be ambiguous. In that case the detector is expected to keep the incumbent, report low confidence, and expose the insufficient score margin rather than claim a reliable exact size.
