...
### Optimization 5.4.1 Result

Auto Suggest grid candidates now use an engine-owned cache key based on asset id, decoded image dimensions, byte length, detector max scale, and preprocessing path. The web app stores candidates under that key and passes reusable source-image candidates through the fix worker request. The core runtime can then resolve `auto` grid settings from the supplied candidates instead of running `detectGridCandidates` a second time during Fix. Background-flood-fill fixes still recompute grid candidates because their detection source is alpha-preprocessed.

| Benchmark | Uncached | Cached | Change |
| --- | ---: | ---: | ---: |
...
| `fake-pixel-720p-single: robust sampled grid detection 0.92MP` | Lazy generated 720p fake-pixel single sprite | Opt-in robust grid detection including blur evidence and conservative reranking | `single` / sprite | 1280x720 source -> 160x90 native target | Report-only | `packages/core/src/fixtureSuite.bench.ts` |
| `robust native-size acceptance matrix: 18 sources` | Six original native-size fixtures plus twelve PixelAid-owned Step 1G failure/control fixtures | Opt-in robust detection across the original inference matrix plus color-field, codec, chroma, ringing, AA, warp, blur, soft-grid, and cell-artifact classes | `single` / sprite | Small generated sources | Advisory 125 ms budget | `packages/core/src/fixtureSuite.bench.ts` |
| `fake-pixel-720p-single: robust hypothesis scoring 0.92MP` | Cached robust candidates on the 720p fake-pixel fixture | Isolated three-candidate provisional reconstruction scoring | `single` / sprite | 1280x720 source -> candidate native grids | Report-only | `packages/core/src/fixtureSuite.bench.ts` |
...
| Advisory | Robust native-size matrix | 18 deterministic Step 1B/Step 1G sources | <= 125 ms mean | `npm run benchmark:record` then `npm run benchmark:budget` | Covers opt-in robust detector evidence and reranking; keep advisory until representative CI timing is stable. |
