...
| `robust native-size acceptance matrix: 6 sources` | Six small procedural native-size inference fixtures | Opt-in robust detection across harmonic, fractional, non-square, softened, drift, and combined classes | `single` / sprite | Small generated sources | Report-only | `packages/core/src/fixtureSuite.bench.ts` |
