...
## First-Party Sample Assets

Onboarding and release demo samples generated by `@pixelaid/fixtures` are first-party PixelAid sample assets authored by Oscar Sanchez. They are deterministic synthetic test/demo materials and are safe to redistribute with PixelAid releases.

These samples are documented in `docs/onboarding-samples.md` and identified in code by the `releaseOnboardingSamples` registry. They do not add third-party asset obligations.
